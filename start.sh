...
connect_to_sshuttle() {
    ...
    if [ -z "$SSH_KEY" ]; then
        sshuttle --dns -r $SSH_USER@$SSH_HOST 0.0.0.0/0 --method tproxy \
            ...
            -x $SSH_HOST -x 127.0.0.1 -x 172.17.0.0/16 -x 172.21.0.0/24 -x 172.22.0.0/24 -x 172.23.0.0/24 -x 10.17.0.0/24 \
            --disable-ipv6
    else
        chmod 600 $SSH_KEY
        sshuttle --dns -r $SSH_USER@$SSH_HOST 0.0.0.0/0 --method tproxy \
            --ssh-cmd "ssh -o "StrictHostKeyChecking=no" -i $SSH_KEY -p $SSH_PORT" \
            -x $SSH_HOST -x 127.0.0.1 -x 172.17.0.0/16 -x 172.21.0.0/24 -x 172.22.0.0/24 -x 172.23.0.0/24 -x 10.17.0.0/24 \
            --disable-ipv6
    fi
